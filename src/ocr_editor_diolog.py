...
class OcrEditorDiolog(QDialog):
    ...
    def setup_reference(self, filename):
        """ 
        精簡版設定：整張圖即為 ROI 與 Offset 區域。
        只需選取：數字錨點 (用於 OCR 拼接增強)
        """
        ...
        # 3. [自動設定] 整張圖即為 ROI 與 Offset 基準
        # 我們將全圖設為 Template，左上角 (0,0) 為基準座標
        auto_roi = (0, 0, w_img, h_img)
        auto_offset_template = ref_img.copy()
        auto_anchor_xy = (0, 0)

        # 4. [唯一手動步驟] 選取數字錨點 (Number Anchor)
        QMessageBox.information(self, "設定數字錨點", 
                              "請框選一個【數字】作為 OCR 拼接錨點。\n"
                              "(系統將儲存此數字圖像，用於信心值過低時的增強辨識)")
        ...
        self.ref_data.update({
            ...
            'roi_box': auto_roi,         # 自動設為全圖
            'anchor_offset': auto_offset_template, # 自動設為全圖
            'anchor_xy': auto_anchor_xy, # 自動設為 (0,0)
            'anchor_number': anchor_img 
        })
